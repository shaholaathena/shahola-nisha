...
const OverviewScreen = () => (
  <svg viewBox="0 0 640 380" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-full h-full" aria-hidden="true">
    <rect width="640" height="380" fill="#050D1C" />
    <rect width="640" height="44" fill="#040A16" />
    <rect x="16" y="15" width="60" height="14" rx="3" fill="#3B82F6" opacity="0.4" />
    ...
    <rect x="496" y="12" width="120" height="20" rx="4" fill="#3B82F6" opacity="0.2" />
    {[0,1,2,3].map(i => (
      <g key={i}>
        ...
        <rect x={28 + i * 155} y="87" width="90" height="20" rx="3" fill={i === 0 ? '#3B82F6' : 'rgba(255,255,255,0.22)'} opacity={i === 0 ? 0.65 : 1} />
        <rect x={28 + i * 155} y="112" width="50" height="8" rx="2" fill={i === 0 ? '#3B82F6' : 'rgba(255,255,255,0.08)'} opacity={0.4} />
        {i === 0 && <rect x={130 + i * 155} y="68" width="22" height="10" rx="2" fill="rgba(52,211,153,0.3)" />}
      </g>
    ))}
    ...
    <rect x="80" y="177" width="44" height="8" rx="2" fill="#3B82F6" opacity="0.35" />
    {[0,1,2,3].map(i => <line key={i} x1="28" y1={220 + i * 32} x2="404" y2={220 + i * 32} stroke="rgba(255,255,255,0.04)" strokeWidth="1" />)}
    <polyline points="32,330 82,298 132,306 182,274 232,245 282,256 332,228 382,238 406,232" stroke="#3B82F6" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" opacity="0.9" />
    <polyline points="32,330 82,298 132,306 182,274 232,245 282,256 332,228 382,238 406,232 406,350 32,350" fill="#3B82F6" opacity="0.07" />
    ...
    <circle cx="232" cy="245" r="4" fill="#3B82F6" />
    ...
    {[0,1,2,3,4,5].map(i => (
      <g key={i}>
        ...
        <rect x="546" y={192 + i * 24} width="52" height="9" rx="2" fill={i <= 1 ? '#3B82F6' : 'rgba(255,255,255,0.05)'} opacity={i <= 1 ? 0.4 : 1} />
      </g>
    ))}
  </svg>
)
...
const TransactionsScreen = () => (
  <svg viewBox="0 0 640 380" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-full h-full" aria-hidden="true">
    ...
    <rect x="84" y="15" width="44" height="14" rx="3" fill="#3B82F6" opacity="0.4" />
    ...
    <rect x="348" y="60" width="80" height="28" rx="4" fill="#3B82F6" opacity="0.15" stroke="rgba(59,130,246,0.3)" strokeWidth="1" />
    <rect x="358" y="69" width="40" height="10" rx="2" fill="#3B82F6" opacity="0.5" />
    ...
    {[0,1,2,3,4,5,6,7].map(i => (
      <g key={i}>
        ...
        <rect x="264" y={142 + i * 28} width="52" height="12" rx="3" fill={i < 2 ? 'rgba(52,211,153,0.2)' : i < 5 ? 'rgba(59,130,246,0.15)' : 'rgba(255,255,255,0.06)'} />
        <rect x="272" y={145 + i * 28} width="36" height="6" rx="1.5" fill={i < 2 ? 'rgba(52,211,153,0.5)' : i < 5 ? 'rgba(59,130,246,0.4)' : 'rgba(255,255,255,0.1)'} />
...
const AnalyticsScreen = () => (
  <svg viewBox="0 0 640 380" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-full h-full" aria-hidden="true">
    ...
    <rect x="16" y="15" width="60" height="14" rx="3" fill="rgba(255,255,255,0.07)" />
    <rect x="84" y="15" width="44" height="14" rx="3" fill="rgba(255,255,255,0.07)" />
    <rect x="136" y="15" width="44" height="14" rx="3" fill="#3B82F6" opacity="0.4" />
    ...
    {['Total Volume', 'Avg Transaction', 'Peak Hour', 'Top Category', 'Success Rate'].map((l, i) => (
      <g key={l}>
        <rect x="28" y={94 + i * 46} width="172" height="34" rx="4" fill={i === 0 ? 'rgba(59,130,246,0.1)' : 'rgba(255,255,255,0.03)'} stroke={i === 0 ? 'rgba(59,130,246,0.25)' : 'rgba(255,255,255,0.06)'} strokeWidth="1" />
        <rect x="36" y={100 + i * 46} width="60" height="7" rx="1.5" fill="rgba(255,255,255,0.1)" />
        <rect x="36" y={112 + i * 46} width="90" height="10" rx="2" fill={i === 0 ? '#3B82F6' : 'rgba(255,255,255,0.2)'} opacity={i === 0 ? 0.6 : 1} />
      </g>
    ))}
    ...
    {[32, 52, 44, 68, 58, 74, 70, 88, 80, 92, 86, 96].map((h, i) => (
      <rect key={i} x={248 + i * 30} y={240 - h} width="20" height={h} rx="2" fill={i >= 9 ? '#3B82F6' : 'rgba(255,255,255,0.08)'} opacity={i >= 9 ? 0.7 : 1} />
    ))}
    ...
    {[0,1,2].map(i => (
      <g key={i}>
        ...
        <rect x={240 + i * 128} y="326" width="70" height="22" rx="3" fill={i === 0 ? '#3B82F6' : 'rgba(255,255,255,0.06)'} opacity={i === 0 ? 0.3 : 1} />
      </g>
    ))}
  </svg>
)
...
const AlertsScreen = () => (
  <svg viewBox="0 0 640 380" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-full h-full" aria-hidden="true">
    ...
    <rect x="84" y="15" width="44" height="14" rx="3" fill="rgba(255,255,255,0.07)" />
    <rect x="136" y="15" width="44" height="14" rx="3" fill="rgba(255,255,255,0.07)" />
    <rect x="188" y="15" width="44" height="14" rx="3" fill="#3B82F6" opacity="0.4" />
    {[
      { color: 'rgba(239,68,68,0.15)', border: 'rgba(239,68,68,0.3)', dot: 'rgba(239,68,68,0.8)', label: 'High — Velocity spike' },
      { color: 'rgba(245,158,11,0.1)', border: 'rgba(245,158,11,0.25)', dot: 'rgba(245,158,11,0.7)', label: 'Medium — Off-hours activity' },
      { color: 'rgba(59,130,246,0.08)', border: 'rgba(59,130,246,0.2)', dot: 'rgba(59,130,246,0.6)', label: 'Info — New device login' },
      { color: 'rgba(52,211,153,0.08)', border: 'rgba(52,211,153,0.2)', dot: 'rgba(52,211,153,0.6)', label: 'Resolved — Threshold breach' },
    ].map((alert, i) => (
      <g key={i}>
    ...
    <rect x="500" y="304" width="108" height="28" rx="4" fill="#3B82F6" opacity="0.25" />
    <rect x="510" y="312" width="80" height="8" rx="2" fill="#3B82F6" opacity="0.6" />
  </svg>
)
...
export default function ImageShowcase() {
  ...
  return (
    <div className="card-surface overflow-hidden">
      ...
      {/* Screen */}
      <div className="relative overflow-hidden bg-[#050D1C]" style={{ height: 320 }}>
