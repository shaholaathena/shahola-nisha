...
export function FloatingNotification({ message, time, type = 'info', delay = 0 }) {
  const typeStyles = {
    ...
    success: { dot: 'bg-zinc-400', border: 'border-zinc-500/20' },
    warning: { dot: 'bg-zinc-300', border: 'border-zinc-400/20' },
  }
...
export function FloatingMetric({ label, value, trend }) {
  return (
    ...
    >
      ...
      {trend && (
        <div className="text-[10px] text-zinc-500 mt-1 font-medium">{trend}</div>
      )}
    </motion.div>
  )
}
