...
export const projects = [
  ...
  },
  {
    ...
    type: 'mobile',
    year: '2026',
    tags: ['Merchant App', 'Bangla QR', 'Multi-bank White-label', 'Onboarding', 'Payments', 'Figma'],
    size: 'medium',
    impact: '5 bank skins',
    ...
    caseStudy: {
      ...
      challenge: 'SSL needed one merchant app that does two jobs — onboard a new merchant through a guided sign-up, and let that merchant accept Bangla QR payments (Bangladesh Bank\'s interoperable QR) — and it had to ship for several banks at once. Each bank wanted its own branded app, but rebuilding the same flows per client wasn\'t sustainable. It also had to stay simple enough for everyday shopkeepers while keeping access secure with PIN and OTP.',
      outcome: 'A single white-label engine reskinned across 5 banks (SEBL, NCC, Rupali, Janata, and SDBL) — covering merchant onboarding (shop info → review → success), Bangla QR payment acceptance (select store & counter → enter amount → scan & pay → success), and transaction history and profile, all behind PIN + OTP auth. Each bank is a re-theme via shared brand tokens, not a redesign.',
      phases: ['Research', 'Design System', 'Mobile UI Design', 'Multi-brand Theming', 'Handoff'],
      metrics: [
        { label: 'Bank skins shipped', value: '5' },
        { label: 'Shared engine', value: '1 core' },
        { label: 'Screens delivered', value: 'TBD' },
        { label: 'Platform', value: 'Android & iOS' },
      ],
      // One SSL base engine, re-skinned per bank client. `fullName` left empty
      // where the official name still needs confirming — abbreviation renders alone.
      banks: [
        ...
        { code: 'Rupali', fullName: 'Rupali Bank',    accent: '#1B7A43', status: 'Live' },
        { code: 'JBL',    fullName: 'Janata Bank',    accent: '#C8102E', status: 'Live' },
      ],
      keyDecisions: [
        {
          title: 'One engine, many banks',
          description: 'Identical screens — splash, login, home, create-lead, take-payment — reskinned per bank through a shared design system, so a new bank client is a re-theme, not a redesign.',
        },
        {
          title: 'Two jobs in one app',
          description: 'Merchant onboarding and Bangla QR payment acceptance live under one navigation, so a merchant signs up and starts taking payments without switching tools.',
        },
        {
          title: 'Guided, low-friction sign-up',
          description: 'The create-lead flow is a short shop-info form → review → success, with minimal fields so first-time shopkeepers get through it quickly.',
        },
        {
          title: 'Bangla QR interoperability',
          description: 'Payments use Bangladesh Bank\'s interoperable QR, so any customer wallet or bank app can scan and pay one merchant QR.',
        },
        {
          title: 'Layered but simple auth',
          description: 'PIN login with OTP verification, plus forget / reset-PIN and quick-signup flows — security matched to a payment app without slowing daily use.',
        },
        {
          title: 'Store & counter model',
          description: 'Merchants select a store and counter before taking a payment, so multi-outlet businesses attribute each transaction correctly.',
        },
      ],
      flowGroups: [
        { label: 'Authentication', items: ['Splash', 'Login', 'Verify OTP', 'Forget PIN', 'Reset PIN', 'Quick Signup'] },
        { label: 'Onboarding', items: ['Onboarding Walkthrough', 'Create Lead — Shop Info', 'Review', 'Success'] },
        { label: 'Store Setup', items: ['Select Store', 'Select Counter'] },
        { label: 'Take Payment', items: ['Enter Amount', 'Scan & Pay (Bangla QR)', 'Success / Failed'] },
        { label: 'Account', items: ['Home', 'Transaction List', 'Profile'] },
      ],
    }
  },
  {
