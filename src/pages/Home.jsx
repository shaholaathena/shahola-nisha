...
function SmallLinkTile({ href, label, children }) {
  return <motion.a href={href} className="group flex min-h-[170px] flex-col justify-between rounded-[28px] border border-black/[.07] bg-[#faf9f6] p-6" whileHover={{ y: -4 }} transition={{ duration: .25 }}><div className="flex items-center justify-between"><span className="text-[10px] font-medium uppercase tracking-[.16em] text-black/40">{label}</span><span className="transition-transform duration-300 group-hover:translate-x-1 group-hover:-translate-y-1">↗</span></div><div className="font-display text-2xl font-medium tracking-[-.045em]">{children}</div></motion.a>
}

function SideQuestTile() {
  return <Tile className="min-h-[170px] bg-[#e9e5ff]"><div className="flex h-full flex-col justify-between p-6"><div className="flex items-center justify-between text-[10px] uppercase tracking-[.16em] text-black/45"><span>Side quests</span><span>✦</span></div><p className="font-display text-2xl font-medium tracking-[-.05em]">Design systems,<br />experiments & tiny ideas.</p></div></Tile>
}
  ...
  return <div className="min-h-screen bg-[#eeece8] text-[#171717] antialiased">
    ...
    <main id="top" className="mx-auto max-w-[1180px] px-4 pb-8 pt-4 sm:px-6 sm:pt-6">
      <div className="grid auto-rows-[170px] grid-cols-1 gap-4 md:grid-cols-12">
        <IntroTile /><SmallLinkTile href="mailto:snisha.athena@gmail.com" label="Email">Let’s talk.</SmallLinkTile><SmallLinkTile href="https://www.linkedin.com/in/shahola-nisha/" label="LinkedIn">Connect with me.</SmallLinkTile><SideQuestTile /><SmallLinkTile href="#work" label="Currently">Designing products.</SmallLinkTile>
