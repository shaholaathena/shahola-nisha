...
function Gate({ text }) {
  return (
    <div className="rounded-xl border-2 border-dashed border-hero-hot/40 bg-hero-hot/[0.05] p-4">
      ...
      <p className="text-[13px] text-ink-secondary leading-relaxed">{text}</p>
    </div>
  )
}
...
function BrowserFrame({ src, alt = '', label = 'zcommerz', tall = false }) {
  return (
    <div className="w-full rounded-xl overflow-hidden border border-border-default bg-hero-void/70 shadow-[0_30px_60px_-30px_rgba(0,0,0,0.6)]">
      {/* chrome */}
      <div className="flex items-center gap-2 px-3.5 h-9 bg-surface-2 border-b border-border-subtle">
        <span className="w-2.5 h-2.5 rounded-full bg-white/15" />
        <span className="w-2.5 h-2.5 rounded-full bg-white/15" />
        <span className="w-2.5 h-2.5 rounded-full bg-white/15" />
        <div className="ml-2 flex-1 h-5 rounded-md bg-white/[0.04] border border-border-subtle flex items-center px-2.5">
          <span className="text-[9px] font-mono text-ink-muted truncate">{label}.zcommerz.app</span>
        </div>
      </div>
      {/* viewport */}
      <div className="relative bg-surface-1 overflow-hidden" style={{ aspectRatio: tall ? '16 / 13' : '16 / 10' }}>
        {src ? (
          ...
          <div className="w-full h-full flex items-center justify-center relative">
            <svg className="absolute inset-0 w-full h-full text-white/10" preserveAspectRatio="none" viewBox="0 0 100 100">
            ...
            <span className="relative text-[9px] font-mono uppercase tracking-[0.18em] text-ink-muted">Wireframe</span>
          </div>
        )}
      </div>
    </div>
          ...
          {HERO_STEPS.map((s, i) => (
            <div key={s} className="flex items-center gap-3">
              ...
              <div className="relative flex-1 h-[10px] rounded-full bg-surface-3 overflow-hidden">
                <div
                  className={`zc-fill absolute inset-y-0 left-0 w-full rounded-full ${i === HERO_STEPS.length - 1 ? 'bg-hero-hot' : 'bg-hero-hot/60'}`}
              ...
              <li key={s.file}>
                <button
                  ...
                  className={`group w-full text-left rounded-xl border px-4 py-3.5 transition-all duration-200 ${
                    on
                      ? 'border-hero-hot/40 bg-hero-hot/[0.08]'
                      : 'border-border-subtle bg-surface-base hover:border-border-strong'
                  }`}
                >
                  <div className="flex items-center gap-3">
                    <span className={`text-[10px] font-mono tabular-nums ${on ? 'text-hero-hot' : 'text-ink-muted'}`}>
                      {String(i + 1).padStart(2, '0')}
                    </span>
                    <span className={`text-[10px] font-mono uppercase tracking-[0.16em] ${on ? 'text-hero-hot/80' : 'text-ink-muted'}`}>
                      {s.step}
                    </span>
                  </div>
                  <div className={`mt-1.5 text-[15px] font-semibold leading-snug ${'text-ink-primary'}`}>
                    {s.title}
                  </div>
                </button>
              </li>
  ...
  return (
    <div className="theme-night min-h-screen bg-hero-void text-ink-primary antialiased">
      ...
      <main className="pt-24 lg:pt-28">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 xl:gap-16 items-center">
              <div className="lg:col-span-7">
                ...
                >
                  ...
                  <span className="w-1 h-1 rounded-full bg-zinc-300" />
                  <span className="text-[11px] font-mono uppercase tracking-[0.2em] text-ink-secondary">E-commerce · SaaS</span>
                </motion.div>
                ...
                <motion.div
                  ...
                  className="text-[15px] font-mono uppercase tracking-[0.18em] text-hero-hot mb-6"
                >
                  {cs.subtitle}
                </motion.div>
        ...
        <section id="cs-journey" style={{ scrollMarginTop: '64px' }}>
          <div className="max-w-[1440px] mx-auto px-6 lg:px-10 py-20 lg:py-24">
        ...
        <section id="cs-store" style={{ scrollMarginTop: '64px' }}>
          <div className="max-w-[1440px] mx-auto px-6 lg:px-10 py-20 lg:py-24">
            <Reveal className="max-w-2xl mb-12">
        ...
        <section id="cs-payments" style={{ scrollMarginTop: '64px' }}>
          <div className="max-w-[1440px] mx-auto px-6 lg:px-10 py-20 lg:py-24">
            <Reveal className="max-w-2xl mb-12">
        ...
        {/* ══════════ 06 DECISIONS ══════════ */}
        <section id="cs-decisions" style={{ scrollMarginTop: '64px' }}>
          <div className="max-w-[1440px] mx-auto px-6 lg:px-10 py-20 lg:py-24">
            <Reveal className="max-w-2xl mb-12">
        ...
        {/* ══════════ 07 DESIGNED + BUILT ══════════ */}
        <section id="cs-build" style={{ scrollMarginTop: '64px' }}>
          <div className="max-w-[1440px] mx-auto px-6 lg:px-10 py-20 lg:py-24">
                  ...
                  {cs.buildPoints.map((p, i) => (
                    <Reveal key={p} delay={i * 0.07}>
                      <li className="flex items-start gap-3">
                        <svg width="15" height="15" viewBox="0 0 16 16" fill="none" className="shrink-0 mt-1 text-hero-hot">
